...
```{r}
htmlBooks<-tbl_df(as.data.frame(read_html("books.html") %>% html_table(header = NA, trim=TRUE, fill=TRUE)))
...
```{r}
xmlBooks<-tbl_df(xmlToDataFrame("books.xml"))
...
jsonbooks<-tbl_df(as.data.frame(fromJSON("books.json")))
